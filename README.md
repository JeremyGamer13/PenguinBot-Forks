...
3. Install Git
4. Create a `cache`, `databases`, `data`, `temp`, folder in the root folder for PenguinBot.
...
- [Object Detection with Grounding DINO](https://huggingface.co/IDEA-Research/grounding-dino-base) (used in commands) (requires python)
    ...
    - Convoluted setup:
        - Install Python 3.10 specifically (other verisons dont seem to work well)
            - You may want to set it up so you can use `py -3.10` if you already have another version installed
        - install `torch`, `transformers`, `pillow`, `thefuzz`
            - On NVIDIA GPUs you will want to use the CUDA versions of `torch`
...
- MIDI rendering with [FluidSynth](https://www.fluidsynth.org/) (used in commands)
    - Configured via `.env`.
    - Setup:
