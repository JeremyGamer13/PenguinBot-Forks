...
class Command {
    ...
    async handle(message, args, util) {
        ...
        await temporaryFolder.createAndDestroy(async (tempDir) => {
            ...
            // normalize
            const outputMixedNormalizePath = path.join(tempDir, `repitch_mixed_normalize.ogg`);
            await FFmpegUtil.commands.normalize(outputMixedPath, outputMixedNormalizePath);

            // done
            await replyMessage.edit({
                content: "Completed in " + ((Date.now() - startTime) / 1000) + " seconds"
                    + "\n" + `-# Generated by <@${message.author.id}>`,
                files: [outputMixedNormalizePath]
            });
        });
    }
